...
const { compareToTarget, formatTime, parseFormattedTime, getDataQualityIssues } = require('./shared-metrics');
const { generateAnalystReportHTML } = require('./monthly-analyst-html-fixed');
 ...
 * NOTE: This function has been replaced by the fixed version in monthly-analyst-html-fixed.js
 * Keeping old version commented for reference
 */
/*
function generateAnalystReportHTML_OLD(currentMetrics, previousMetrics) {
...
*/
// End of old function - now using fixed version from monthly-analyst-html-fixed.js
...
/**
 * Fetch monthly metrics data from cache
 * Loads real data generated by ./run-monthly.sh
 */
async function fetchMonthlyMetrics() {
  console.log('Loading monthly metrics from cache...');
  ...
  try {
    ...
    console.error('\n⚠️  You must run ./run-monthly.sh FIRST to generate metrics data.');
    console.error('   The analyst report consumes data from the monthly dashboard script.');
    process.exit(1);
  }
}
...
async function main() {
  ...
  const metricsData = await fetchMonthlyMetrics();
